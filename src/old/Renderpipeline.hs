...
-- the game is rendered in coords where (0,0) is the left upper corner of the field
-- the debug info is rendered in coords where (0,0) is the left upper corner of the window
renderWorld :: SizeOnScreen -> AreaOnScreen -> World -> Picture
renderWorld windowSize areaOnScreen@(posField,_) world = case (uiState world) of -- check wether the game is currently in status Playing or Menu
	Playing ->  Pictures $  [
		onField $ renderPlaying areaOnScreen world,
		renderDbgInfo (dbgInfo world) textArea]
			where
				textArea = ((20,20),posField)
				onField pic = (uncurry Translate) (i2C areaOnScreen (fromIntegral 0, fromIntegral 0)) $ -- Translation
...
renderDbgInfo :: DebugInfo -> AreaOnScreen -> Picture
renderDbgInfo DbgInf{ info=infTxt } (pos,size) =
	(uncurry Translate) (i2C (pos,size) (0,0)) $
	--Scale 1 (-1) $
	Color white $ 
	Text infTxt
